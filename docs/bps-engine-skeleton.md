...
> **⚠️ 历史文档**：本文档描述的是 2026-02-23 的初始设计，早于 2026-03-03 的架构瘦身。
> 文中的 ProcessManager、RuleEvaluator、SysCall、ContextAssembler、ConflictDetector 等模块
> 已在引擎瘦身（ADR-10）中移除。当前实际架构请参见 README.md。
...
│   │
│   ├── knowledge/            ← BKM 业务知识管理子系统
│   │   ├── types.ts          ← KnowledgeLayer/Scope/Entry/ConflictReport 类型
│   │   ├── knowledge-store.ts ← 知识 CRUD（封装 DossierStore，entityType="knowledge"）
│   │   ├── context-assembler.ts ← Scope chain 构建 + 知识装配 + 浅合并
│   │   ├── conflict-detector.ts ← 字段级冲突检测（severity 分级）
...
```yaml
# idlex-blueprint.yaml — IdleX业务蓝图示例（片段）
version: "1.0"
name: "IdleX・自助空间AI交易平台"
...
1. 定义IdleX Entity 清单（门店、时空SKU、用户、订单…）
2. 定义IdleX Service 清单（门店入驻、GEO发布、订单处理…）
3. 定义IdleX Rule 清单（业务流程编排）
4. 定义IdleX Skill 清单（Agent 能力映射）
5. 端到端测试
