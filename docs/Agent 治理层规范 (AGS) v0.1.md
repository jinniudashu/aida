# Agent 管理层规范 (Agent Management Specification - AMS) v0.1

> 状态：草案
> 日期：2026-03-05
> 前置文档：`archive/Blueprint管理层讨论纪要 (2026-03-05).md`
...
管理层填补这个空白：在 Agent 和工具之间插入一个**机械的、不可绕过的约束检查层**。

## 2. 核心概念

### 2.1 运营层 vs 管理层

| | 运营层 (Operations) | 管理层 (Management) |
|---|---|---|
| 职责 | Agent **应该做**什么 | Agent **不能做**什么 |
| 执行者 | Aida（AI 自主决策） | Management Engine（零 AI 裁量权） |
| 时机 | 事后记录 | **事前拦截** |
...
| **Policy** | 一组 Constraints 的集合，构成某个管理领域的完整约束集 |
...
在 `management.yaml` 的 `circuit_breaker.thresholds` 中定义：
...
1. 记录完整的违规链到 DossierStore（`entityType="management-incident"`）
2. 通过 OpenClaw 事件系统发射 `bps.management.disconnect` 事件
...
Violation 记录存入专用 SQLite 表 `bps_management_violations`（非 DossierStore）：
...
Phase C 的 Dashboard Approvals 页面已经实现了审批队列。管理层的 REQUIRE_APPROVAL 与此自然衔接：
...
CREATE TABLE IF NOT EXISTS bps_management_violations (
  ...
  created_at TEXT NOT NULL,
  FOREIGN KEY (constraint_id) REFERENCES bps_management_constraints(id)
);
...
CREATE INDEX idx_violations_time ON bps_management_violations(created_at);
CREATE INDEX idx_violations_severity ON bps_management_violations(severity, created_at);
CREATE INDEX idx_approvals_status ON bps_management_approvals(status);
```

## 9. management.yaml 加载

### 9.1 文件位置

```
~/.aida/
├── management.yaml           # 管理宪法（新增）
├── project.yaml
├── blueprints/
├── data/
└── context/
...
`loadAidaProject()` 扩展：在加载 project.yaml 和 blueprints 之后，加载 management.yaml。
...
management.yaml 变更时支持热重载（无需重启 OpenClaw）：
- 新增 BPS tool `bps_reload_management`（或 Dashboard API `POST /api/management/reload`）
    ...
    async execute(callId: string, input: unknown) {
      ...
      if (result.verdict === 'BLOCK') {
        return {
          success: false,
          management_blocked: true,
          reason: 'Action blocked by management constraint.',
          violations: result.checks.filter(c => !c.passed),
        };
      }
      ...
      if (result.verdict === 'REQUIRE_APPROVAL') {
        ...
        return {
          success: false,
          management_pending_approval: true,
          approvalId,
...
| # | Tool | 说明 |
|---|------|------|
| 13 | `bps_management_status` | 查询当前 Circuit Breaker 状态 + 最近违规记录 |

此工具为只读，不受 Action Gate 拦截：

```typescript
// bps_management_status: Agent 可以查询自身的管理状态
{
...
| Method | Path | 说明 |
|--------|------|------|
| `GET` | `/api/management/status` | Circuit Breaker 状态 + 统计 |
| `GET` | `/api/management/violations` | 违规记录列表（支持 limit 参数） |
| `GET` | `/api/management/constraints` | 当前活跃的约束列表 |
| `GET` | `/api/management/approvals` | 管理层待审批列表 |
| `POST` | `/api/management/approvals/:id/decide` | 审批/拒绝（附带 decision + reason） |
| `POST` | `/api/management/circuit-breaker/reset` | 重置 Circuit Breaker 为 NORMAL |

## 11. 实现优先级

### Phase E1: 核心 Gate（最小可用）

1. `management.yaml` schema 定义 + 解析器
2. `ManagementStore`（约束加载 + 违规记录 + Circuit Breaker 状态）
3. `ActionGate`（scope 匹配 + expr-eval 求值 + verdict 判定）
4. `wrapWithManagement()` 包装 `bps_update_entity`
5. `bps_management_status` tool
...
3. Dashboard Management Panel（违规列表 + 熔断器状态 + 重置按钮）
4. 审批超时处理
5. 测试：熔断阈值、审批流程、状态恢复

### Phase E3: 运营化

1. `loadAidaProject()` 集成（自动加载 management.yaml）
2. 热重载支持
3. Aida AGENTS.md 更新（Agent 感知管理层的存在）
4. install-aida.sh 更新（management.yaml 示例部署）
5. 晨光咖啡场景的管理约束示例
...
- **不做运行时流程编排**。管理层只判断 PASS/BLOCK/REQUIRE_APPROVAL，不指导 Agent 该做什么。
- **不引入 LLM 判断**。所有 condition 必须是 expr-eval 可求值的确定性表达式。模糊的管理规则不是管理规则。
- **不替代 OpenClaw 的权限系统**。管理层是业务语义的约束，不是技术层面的 ACL。两者互补。
- **不追求覆盖所有工具**。初始版本只拦截写操作。只读操作天然安全。

### 12.2 关键不变量

1. **机械性**：Management Engine 的代码路径中没有 LLM 调用，没有概率判断，没有"也许"。
...
| BPS 概念 | 管理层中的角色 | 说明 |
|----------|-------------|------|
| Blueprint YAML | 被 management.yaml 替代其管理职责 | Blueprint 的"正向编排"价值由 Agent 自主完成；"负向约束"价值由管理层接管 |
