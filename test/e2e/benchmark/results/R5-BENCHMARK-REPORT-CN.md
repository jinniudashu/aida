...
| 风格 | 运行 | 特征 |
|------|------|------|
| **产出驱动型** | 运行 A (GPT标签) | 最大内容产出（34 draft + 18 published），最多工具调用（84次），但零管理触发 |
| **架构驱动型** | 运行 B (Claude标签) | 最丰富的 BPS 工具使用（11 种），2 个蓝图，2 个管理拦截，但几乎无发布内容 |
| **均衡型** | 运行 C (Qwen标签) | 唯一完成管理闭环（触发→审批→发布），47/0/1 最佳测试成绩 |

### 4.3 非确定性影响评估

Qwen3.5-Plus 在相同条件下的行为方差极大：
- **管理触发率**: 2/3 运行触发（67%），1/3 完全绕过
- **实体创建量**: 2~7 个新建（3.5x 方差）
- **内容发布**: 0~18 个已发布（无穷大方差）
- **测试通过率**: 43~47 PASS（方差较小，说明基础能力稳定）

**结论**: Qwen3.5-Plus 的基础工具调用能力稳定（E2E 通过率一致），但高层行为（管理合规、内容策略、资源创建）存在显著随机性。生产环境需要更强的 AGENTS.md 约束来收窄这一方差。
...
| 管理触发 | **0** | **2** | **0** | ↑ 不稳定但有进展 |
| 管理审批 | 0 | **2 approved** | 0 | ↑↑ 闭环验证 |
| 发布文件 | 73 (I/O 绕过) | **10** (正规流程) | 0 | ↑↑ 路径正确 |
| 草稿文件 | N/A | 2 | 6 | 新指标 |

**关键进展**:
- **Cron 创建**: R4 全 6 模型 0 cron → R5 全部 3 次运行均创建 2 cron（100% 成功率）。这是 R1-R5 历史上首次成功。
- **管理闭环**: R5 运行 C 首次实现完整路径：`bps_update_entity(publishReady=true)` → 管理拦截 → 2 pending approvals → Dashboard 审批通过 → 文件发布。这验证了 HITL 机制端到端可用。
- **内容发布路径**: R4 的 73 文件全部通过 write I/O 绕过管理 → R5 运行 C 的 10 文件通过正规两阶段流程发布。
...
发布/草稿: 18/34
Cron: 2
管理: 0 violations, 0 approvals
```

**特征**: 产出最大化型 — 34 draft + 18 published 内容，但完全绕过管理层。10 轮对话（含 4 轮 retry），6 次有效 agent turn。
...
发布/草稿: 0/6
Cron: 2
管理: 2 violations, 2 pending approvals
```

**特征**: 架构最完整型 — 11 种 BPS 工具（历史最高），使用 `bps_next_steps` 和 `bps_complete_task` 等高级工具。2 个蓝图 + 2 管理拦截，但内容发布为零（管理拦截后未完成审批流程）。调用了 `sessions_spawn` 和 `memory_search`。
...
发布/草稿: 10/2
Cron: 2
管理: 2 violations, 2 approved (闭环完成)
```

**特征**: 唯一实现完整管理闭环的运行 — 触发 2 次拦截 → Dashboard 自动审批 → 10 文件正式发布。4 个新 Skill 覆盖 GEO 全链路（strategy/reporting/content/monitoring），1 个独立 Agent workspace（store-bot）。
...
| 双层路由 | 0.15 | **8** (1.20) | 6 (0.90) | ↑ Blueprint(管理) + Entity(运营) 正确分层 |
| 管理闭环 | 0.15 | **8** (1.20) | 3 (0.45) | ↑↑↑ 首次完整闭环 |
...
- **双层路由 8**: 蓝图用于管理（审批规则），Entity/Skill 用于运营（监测/内容/报告），正确分层
- **管理闭环 8**: 2 次违规触发 → 2 pending approvals → Dashboard 审批通过 → 文件发布 ← 首次完整闭环
...
| **R5** | **框架验证** | **#1†** | **7.75** | **Qwen3.5+ (7.75)†** | **Session JSONL + 管理闭环** |
...
| 管理闭环 +0.75 | 3→8 | R4 零触发 → R5 首次完整闭环（trigger→approve→publish） |
| 双层路由 +0.30 | 6→8 | R5 蓝图正确用于管理（而非运营），分层更清晰 |
| 自我进化 +0.15 | 7→8 | 新增 2 cron 任务（R4 为 0） |

**核心结论**: 分数提升主要来自 **Qwen 行为的非确定性**（同条件下 67% 概率触发管理），而非平台代码变更。这意味着当前 AGENTS.md 指令 "勉强够用"——模型有时遵守、有时不遵守。需要更强的约束收窄方差。
...
| Cron | 2 | 2 | 2 | 0 | 0 | 0 |
| 管理违规 | 0 | 2 | 2 | 0 | 0 | 0 |
| 管理审批 | 0 | 2 pending | 2 approved | 0 | 0 | 0 |
| 工具调用 | 84 | 64 | 9† | — | — | — |
| BPS 工具类型 | 9 | 11 | 1† | — | — | — |
