...
| 检查 | ~97 checks (R3) | **133 checks** (74 引擎 + 59 其他) |
| 引擎检查 | 39 (D1-D8) | **74** (D1-D8 + Σ1/Σ7/Σ9/Σ10/ΣX/Σ11) |
...
### Phase 2: 引擎测试 — D1-D8 + AEF Σ1-Σ11 (74 checks)
...
| ID | 描述 | 验证内容 |
|----|------|---------|
| E10.01 | REQUIRE_APPROVAL 产生结构化审批记录 | 审批记录含 approvalId + constraintId（人类可溯源） |
| E10.02 | 审批通过后重放成功 | 实体版本递增 + 数据更新（反馈闭环正极） |
| E10.03 | 审批拒绝后不执行 | 实体保持不变（反馈闭环负极） |
| E10.04 | 审批决策反馈到约束效能 | constraintEffectiveness 统计 approved/rejected 计数 |
| E10.05 | Agent 可查询审批历史 | management_status 暴露 constraintEffectiveness 字段 |
