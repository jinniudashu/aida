...
| D4 | 合规约束捕获率 | 0.15 | 知情同意/治疗间隔/剂量上限等约束是否转化为 Governance |
...
| F4 | 会员折扣不叠加 | 明确规则 | SOFT: entity/governance |
...
Round 1 使用 6 turns。Turn 4 合并了知情同意+术后护理+治理约束，导致创建不完整。

### v0.2 脚本（7 Turns, Round 2+ 使用）

将 Turn 4 拆分为 4a（知情同意+治理）和 4b（术后护理），增加明确数量期望。
...
- Governance 约束写入 governance.yaml（≥7 条）
...
#   blueprints: 数量 + 名称
#   governance: constraints/violations/approvals 计数
#   skills: 数量
#   crons: 数量
