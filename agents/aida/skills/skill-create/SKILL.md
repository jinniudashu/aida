...
- [ ] **Management compliance**: if the Skill involves write operations (creating/updating entities, publishing content, outputting files), it must explicitly require `bps_update_entity` before any file I/O — this triggers management review. Never design a Skill that writes output files without a prior entity update step.
...
The tool automatically appends a `## Management` section with active project constraints — you do not need to write it manually. But you must design the Skill's workflow to respect those constraints.
